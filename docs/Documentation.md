...
* Note these points when adapting the documentation to a different project/product:
  ...
  * :bulb: In addition to updating content files, you might have to update the config files `docs\_config.yml` and `docs\_sass\minima\_base.scss` (which contains a reference to `NUS ModBook` that comes into play when converting documentation pages to PDF format).
